...
snippetsRouter.patch('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    // Only owner or Editor can mutate
    ...
    const { tagIds, ...rest } = updateSchema.parse(req.body)
...
snippetsRouter.post('/:id/collaborators', async (req: AuthRequest, res, next) => {
  try {
    ...
    const collab = await prisma.collaborator.upsert({
      where: { snippetId_userId: { snippetId: snippet.id, userId: invitee.id } },
      create: { snippetId: snippet.id, userId: invitee.id, role },
      update: { role },
    })
...
// ── DELETE /api/snippets/:id/collaborators/:userId ─────────────────────────
snippetsRouter.delete('/:id/collaborators/:collabUserId', async (req: AuthRequest, res, next) => {
  try {
    const requesterId = req.user!.userId
    const { id: snippetId, collabUserId } = req.params as { id: string; collabUserId: string }
